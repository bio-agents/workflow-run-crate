...
This profile is used to describe the execution of a computational agent that has orchestrated the execution of other agents. Such a agent is represented as a [workflow](https://www.researchobject.org/ro-crate/1.1/workflows.html) that can be executed using a *workflow engine* (e.g. [cwlagent](https://github.com/common-workflow-language/cwlagent)).
